...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NDINDIRECT){
    ...
    int dindirect = NDIRECT + 1;
    if((addr = ip->addrs[dindirect]) == 0)
      ip->addrs[dindirect] = addr = balloc(ip->dev);
    bp = bread(ip->dev, addr);
    a = (uint*)bp->data;
    //read indirect block
    int fst_ref = bn/NINDIRECT;
    if((addr = a[fst_ref]) == 0){
      a[fst_ref] = addr = balloc(ip->dev);
    }
    dbp = bread(ip->dev, addr);
    a = (uint*)dbp->data;
    //read data block
    int snd_ref = bn % NINDIRECT;
    if((addr = a[snd_ref]) == 0){
      a[snd_ref] = addr = balloc(ip->dev);
      //records the block number in memory to reserve a slot in the log on disk 
      log_write(dbp);
      log_write(bp);
    }
    //release lock for other processes to access the buffer
    brelse(dbp);
    brelse(bp);
    return addr;
  }
...
itrunc(struct inode *ip)
{
  ...
  //double_indirect
  //at first, free data blocks
  //next, free indirect blocks
  //lastly, free double-indirect blocks
