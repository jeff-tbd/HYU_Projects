...
RC SM_Manager::OpenDb(const char *dbName) {
	...
	if (strlen(dbName) > MAXDBNAME || strchr(dbName, ' ')
			|| strchr(dbName, '/')) {
		rc = SM_INVALIDDBNAME;
		goto err_return;
	}
	...
	// Return error
	err_close: pRmm->CloseFile(fhRelcat);
	err_return: return (rc);
}
...
RC SM_Manager::CreateIndex(const char *relName, const char *attrName) {
	...
	// Build index
	if ((rc = pIxm->CreateIndex(relName, indexNo, ((SM_AttrcatRec *)attrcatData)->attrType, ((SM_AttrcatRec *)attrcatData)->attrLength)))
		goto err_return;
	...
	// Return error
	err_closescan: fs.CloseScan();
	err_closefile: pRmm->CloseFile(fh);
	err_closeindex: pIxm->CloseIndex(ih);
	err_destroyindex: pIxm->DestroyIndex(relName, indexNo);
	err_return: return (rc);
}
...
RC SM_Manager::Load(const char *relName, const char *fileName) {
	...
	while (fgets(buf, MAXLINE, fp)) {
		...
		if (buf[i] != '\n' || buf[i+1] != '\0' || numDelim + 1 != ((SM_RelcatRec *)relcatData)->attrCount) {
			rc = SM_INVALIDFORMAT;
			goto err_closeindexes;
		}
	...
	// Return error
	err_closeindexes: for (i = 0; i < ((SM_RelcatRec *)relcatData)->attrCount; i++)
		if (attributes[i].indexNo != -1)
			pIxm->CloseIndex(ihs[i]);
	//err_closefile:
	pRmm->CloseFile(fh);
	err_fclose: fclose(fp);
	err_deletedata: delete [] data;
	err_deletebuf: delete [] buf;
	err_deleteattributes: delete [] attributes;
	err_deleteihs: delete [] ihs;
	err_return: return (rc);
}
...
bool SM_Manager::IsAttrIndexed(const char* relName, const char* attrName) {
	DataAttrInfo * aInfo;
	RM_Record rec;
   GetAttributeInfo(relName, attrName, rec,(char* &)aInfo);
	return aInfo->indexNo != -1 ? true : false;
}
//
// compareDataAttrInfo
//
...
// Ret:  SM_RELNOTFOUND, RM return code
//
RC SM_Manager::GetRelationInfo(const char *relName, RM_Record &rec, char *&data) {
	RC rc;
	char _relName[MAXNAME];
	RM_FileScan fs;
...
RC SM_Manager::GetFromTable(const char *relName, int& attrCount,
		DataAttrInfo *& attributes) {
	RC rc;
	RM_Record tmpRec;
	char *relcatData;
...
RC SM_Manager::InsertRecord(const char *relName, int& attrCount,
		DataAttrInfo *& attributes, const char *data) {
	RC rc;
	RM_FileHandle fh;
	...
	// Return ok
	return (0);
	err_closeindexes: for (i = 0; i < attrCount; i++)
		if (attributes[i].indexNo != -1)
			pIxm->CloseIndex(ihs[i]);
	//err_closefile:
	pRmm->CloseFile(fh);
...
RC SM_Manager::DropIndexFromAttrCatAlone(const char *relName,
                                         const char *attrName)
{
  cout << "attrName " << attrName << " early in DropIndexFromAttrCatAlone\n";
  ...
  DataAttrInfo * data;
  RC rc;
  if ((rc = rfs.OpenScan(fhAttrcat,
                         STRING,
                         MAXNAME+1,
                         OFFSET(DataAttrInfo, relName),
                         EQ_OP,
                         (void*) relName))) 
    return (rc);
  ...
  while (rc!=RM_EOF) {
    ...
    if (rc!=RM_EOF) {
      ...
      cout << "data->attrName " << data->attrName << " found in DropIndexFromAttrCatAlone\n";
  ...
  if ((rc = fhAttrcat.UpdateRec(rec)) != 0)
    return rc;
  return (0);
}
